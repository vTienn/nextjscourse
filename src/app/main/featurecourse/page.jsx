...
const FeatureCourse = () => {
    ...
    return ( <
            ...
            currentCourses.length > 0 ? ( <
                Row gutter = {
                    ...
                    currentCourses.map((course) => ( <
                        Col className = "box-detail-course"
                        key = { course.id }
                        span = { 8 } >
                        <
                        div style = {
                        ...
                        />{' '} <
                        h3 style = {
                            { marginBottom: '8px' } } > { course.name } < /h3>{' '} <
                        div style = {
                            { fontSize: '14px', color: '#777', marginBottom: '10px' } } > { ' ' } {
                            course.duration && ( <
                                p style = {
                        ...
                        /div>{' '} <
                        p style = {
                            { color: '#555', marginTop: 'auto', marginBottom: '15px' } } > { ' ' } { course.description } { ' ' } <
                        /p>{' '} <
                        div style = {
                        ...
                        style = {
                            { flex: 1 } }
                        onClick = {
                            () => alert(`Registering for ${course.name}`) } >
                        Register { ' ' } <
                        /Button>{' '} {
                            course.videoUrl && ( <
        ...
        />{' '} <
        /div>{' '} <
        /div> { /* Modal hiển thị video YouTube */ } { ' ' } <
        Modal title = "Course Demo"
        open = { isModalOpen }
        onCancel = { closeModal }
        footer = { null }
        width = { 800 } > { ' ' } {
            selectedVideo ? ( <
                ...
                <
                iframe src = { selectedVideo.replace('watch?v=', 'embed/') }
